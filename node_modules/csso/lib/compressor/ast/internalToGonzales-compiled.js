...
function toGonzales(node) {
    switch (node.type) {
        case 'StyleSheet':
            return [node.info || {}, 'stylesheet'].concat(node.rules.map(toGonzales).filter(Boolean));
        ...
        case 'Atrule':
            ...
            var result = [node.info, type, [{}, 'atkeyword', [{}, 'ident', node.name]]];

            if (node.expression && !node.expression.sequence.isEmpty()) {
                if (type === 'atruler') {
                    result.push([node.expression.info, 'atrulerq', [{}, 's', ' ']].concat(node.expression.sequence.map(toGonzales)));
                } else {
                    result.push([{}, 's', ' ']);
                ...
                if (type === 'atruler') {
                    result.push([{}, 'atrulerq']);
                }
            }

            if (node.block) {
                if (type === 'atruler') {
                    result.push([node.block.info, 'atrulers'].concat(node.block.rules.map(toGonzales)));
                } else {
        ...
        case 'Ruleset':
            return [node.info, 'ruleset', toGonzales(node.selector), toGonzales(node.block)];

        case 'Selector':
            return eachDelim(node, 'selector', 'selectors', [{}, 'delim']);

        case 'SimpleSelector':
            var result = [node.info, 'simpleselector'];
        ...
        case 'Negation':
            ...
            return [node.info, 'pseudoc', [{}, 'funktion', [{}, 'ident', 'not'], body]];

        case 'Attribute':
            var result = [node.info, 'attrib'];
        ...
        case 'FunctionalPseudo':
            if (/^nth-/.test(node.name)) {
                var result = [node.info, 'nthselector', [{}, 'ident', node.name]];
                ...
                return [node.info, 'pseudoc', [{}, 'funktion', [{}, 'ident', node.name], body]];
            }
        ...
        case 'Function':
            ...
            return [node.info, 'funktion', [{}, 'ident', node.name], body];

        case 'Block':
            return eachDelim(node, 'block', 'declarations', [{}, 'decldelim']);

        case 'Declaration':
            return [node.info, !node.value.sequence.isEmpty() && node.value.sequence.first().type === 'Progid' && /(-[a-z]+-|[\*-_])?filter$/.test(node.property.name) ? 'filter' : 'declaration', toGonzales(node.property), toGonzales(node.value)];

        case 'Braces':
            return [node.info, 'braces', node.open, node.close].concat(node.sequence.map(toGonzales));

        case 'Value':
            var result = [node.info, !node.sequence.isEmpty() && node.sequence.first().type === 'Progid' ? 'filterv' : 'value'].concat(node.sequence.map(toGonzales));
        ...
        case 'Combinator':
            return node.name === ' ' ? [node.info, 's', node.name] : [node.info, 'combinator', node.name];
        ...
        case 'Dimension':
            return [node.info, 'dimension', [{}, 'number', node.value], [{}, 'ident', node.unit]];

        case 'Operator':
            return [node.info, node.value === '+' || node.value === '-' ? 'unary' : 'operator', node.value];
...
//# sourceMappingURL=internalToGonzales-compiled.js.map