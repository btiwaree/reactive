...
function StyleSheet(tokens) {
    ...
    for (var i = 2; i < tokens.length; i++) {
        ...
        if (type !== 's' && type !== 'comment' && type !== 'unknown') {
...
var types = {
    ...
    atruleb: function (token) {
        return Atrule(token, trimSC(token, 3, token.length - 2), convertToInternal(token[token.length - 1]));
    },
    atruler: function (token) {
        return Atrule(token, convertToInternal(token[3]), convertToInternal(token[4]));
    },
    ...
    atrules: function (token) {
        return Atrule(token, trimSC(token, 3, token.length - 1), null);
    },
    ...
    nthselector: function (token) {
        return {
            ...
            arguments: new List([{
                type: 'Argument',
                sequence: new List(token.slice(3).filter(function (item) {
...
//# sourceMappingURL=gonzalesToInternal-compiled.js.map