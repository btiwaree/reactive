...
function parseError(message) {
    ...
    if (tokens.length) {
        if (pos < tokens.length) {
            ...
            column = lines.length > 1 ? lines[lines.length - 1].length + 1 : tokens[pos].column + lines[lines.length - 1].length;
        }
    }
...
function expectIdentifier(name, eat) {
    if (pos < tokens.length) {
        ...
        if (token.type === TokenType.Identifier && token.value.toLowerCase() === name) {
            if (eat) {
                pos++;
            }
        ...
        switch (tokens[pos].type) {
            ...
            case TokenType.LeftCurlyBracket:
                if (isBlockAtrule(pos)) {
                    ...
                    node.push([{}, NodeType.AtrulerqType].concat(node.splice(3)));
...
function getRuleset() {
    return [getInfo(pos), NodeType.RulesetType, getSelector(), getBlockWithBrackets()];
}
...
function getSelector() {
    ...
    scan: while (pos < tokens.length) {
        switch (tokens[pos].type) {
            ...
            case TokenType.Comma:
                selector.push([getInfo(pos++), NodeType.DelimType]);
                break;
    ...
    scan: while (pos < tokens.length) {
        switch (tokens[pos].type) {
            ...
            case TokenType.DecimalNumber:
                node.push(tryGetPercentage() || getNamespacedIdentifier(false));
                break;
...
function getBlock(info) {
    ...
    scan: while (pos < tokens.length) {
        switch (tokens[pos].type) {
            ...
            case TokenType.Semicolon:
                // ;
                node.push([getInfo(pos++), NodeType.DecldelimType]);
                break;
...
function getDeclaration(nested) {
    ...
    for (var j = startPos; j < pos; j++) {
        if (tokens[j].value.toLowerCase() === 'filter') {
            if (checkProgid(pos)) {
                return [info, NodeType.FilterType, property, getFilterv()];
            }
            break;
        }
    }

    return [info, NodeType.DeclarationType, property, getValue(nested)];
}
...
function getProperty() {
    ...
    return readSC([info, NodeType.PropertyType, [info, NodeType.IdentType, name + readIdent(true)]]);
}
        ...
        switch (tokens[pos].type) {
            ...
            default:
                ...
                if (tokens[pos].type === TokenType.Identifier) {
                    ...
                    if ((prefix === 'U' || prefix === 'u') && pos + 1 < tokens.length && tokens[pos + 1].type === TokenType.PlusSign) {
                        pos += 2;

                        node.push([getInfo(pos), NodeType.IdentType, prefix + '+' + getUnicodeRange(true)]);
                        break;
                    }
                }
...
function getAny(scope) {
    ...
    switch (tokens[pos].type) {
        ...
        case TokenType.PlusSign:
            ...
            if (tokens[pos].type === TokenType.HyphenMinus && pos < tokens.length && (tokens[pos + 1].type === TokenType.Identifier || tokens[pos + 1].type === TokenType.HyphenMinus)) {
                break;
            }

            if (tokens[pos].type === TokenType.HyphenMinus || tokens[pos].type === TokenType.PlusSign) {
                return getUnary();
            }
...
function getAttribute() {
    ...
    if (pos < tokens.length && tokens[pos].type !== TokenType.RightSquareBracket) {
        ...
        if (pos < tokens.length && tokens[pos].type === TokenType.Identifier) {
            node.push([getInfo(pos), 'attribFlags', tokens[pos++].value]);
            readSC(node);
        }
    }
...
function getBraces(scope) {
    expectAny('Parenthesis or square bracket', TokenType.LeftParenthesis, TokenType.LeftSquareBracket);
    ...
    var node = [getInfo(pos), NodeType.BracesType, tokens[pos].value, null];
...
function getClass() {
    ...
    return [getInfo(startPos), NodeType.ClassType, getIdentifier()];
}
...
function getShash() {
    ...
    return [getInfo(startPos), NodeType.ShashType, readIdent()];
}
...
function getDimension(startPos, number) {
    return [getInfo(startPos || pos), NodeType.DimensionType, number || getNumber(), [getInfo(pos), NodeType.IdentType, readUnit()]];
}
...
function getOldIEExpression(startPos, ident) {
    ...
    return [getInfo(startPos), NodeType.FunctionExpressionType, raw];
}
    ...
    scan: while (pos < tokens.length) {
        switch (tokens[pos].type) {
            ...
            case TokenType.Comma:
                ...
                wasSelector = false;
                node.push([getInfo(pos++), NodeType.DelimType]);
                break;
...
function getVarFunctionBody() {
    ...
    if (pos < tokens.length && tokens[pos].type === TokenType.Comma) {
        node.push(getOperator(), getValue(true));
        readSC(node);
    }
...
function getUri() {
    ...
    if (tokens[pos].type === TokenType.String) {
        ...
        node.push([getInfo(rawStart), NodeType.RawType, raw]);
...
function getUnicodeRange(tryNext) {
    ...
    for (; pos < tokens.length; pos++) {
        if (tokens[pos].type !== TokenType.DecimalNumber && tokens[pos].type !== TokenType.Identifier) {
            break;
        }
...
function getNamespacedIdentifier(checkColon) {
    ...
    if (pos < tokens.length) {
        if (tokens[pos].type === TokenType.VerticalLine && pos + 1 < tokens.length && tokens[pos + 1].type !== TokenType.EqualsSign) {
            name += '|';
            pos++;

            if (pos < tokens.length) {
                if (tokens[pos].type === TokenType.HyphenMinus || tokens[pos].type === TokenType.Identifier || tokens[pos].type === TokenType.LowLine) {
                    name += readIdent();
    ...
    return [info, NodeType.IdentType, name];
}
...
function getNth() {
    ...
    if (tokens[pos].type === TokenType.DecimalNumber) {
        if (pos + 1 < tokens.length && tokens[pos + 1].type === TokenType.Identifier && tokens[pos + 1].value.toLowerCase() === 'n') {
    ...
    return [getInfo(startPos), NodeType.NthType, value];
}
...
function getOperator() {
    expectAny('Operator', TokenType.Solidus, TokenType.Asterisk, TokenType.Comma, TokenType.Colon, TokenType.EqualsSign);
...
function checkSC(i) {
    ...
    while (i < tokens.length) {
        if (tokens[i].type === TokenType.Space || tokens[i].type === TokenType.Comment) {
            i++;
        } else {
            break;
        }
...
function checkProgid(i) {
    ...
    if (i + 1 >= tokens.length || tokens[i + 0].value.toLowerCase() !== 'progid' || tokens[i + 1].type !== TokenType.Colon) {
        return false; // fail
    }
    ...
    if (i + 6 >= tokens.length || tokens[i + 0].value.toLowerCase() !== 'dximagetransform' || tokens[i + 1].type !== TokenType.FullStop || tokens[i + 2].value.toLowerCase() !== 'microsoft' || tokens[i + 3].type !== TokenType.FullStop || tokens[i + 4].type !== TokenType.Identifier) {
        return false; // fail
    }
    ...
    if (i >= tokens.length || tokens[i].type !== TokenType.LeftParenthesis) {
        return false; // fail
    }
...
function getProgid() {
    ...
    node.push([getInfo(rawStart), NodeType.RawType, value]);
...
function getPseudoClass() {
    ...
    return [getInfo(startPos), NodeType.PseudocType, node];
}
...
// '+' | '-'
function getUnary() {
    expectAny('Unary operator', TokenType.HyphenMinus, TokenType.PlusSign);
