          ...
          if (!sourceMap.sourcesContent[i]) {
            ...
            if (absPath === file.path) {
              ...
              // else load content from file
                ...
                try {
                  if (options.debug) console.log(PLUGIN_NAME + '-init: No source content for "' + source + '". Loading from file.');
                  sourceContent = stripBom(fs.readFileSync(absPath, 'utf8'));
                } catch (e) {
                  if (options.debug) console.warn(PLUGIN_NAME + '-init: source file not found: ' + absPath);
                }
              }
...
module.exports.write = function write(destPath, options) {
  ...
  function sourceMapWrite(file, encoding, callback) {
    ...
    if (options.includeContent) {
      ...
      for (var i = 0; i < file.sourceMap.sources.length; i++) {
        if (!sourceMap.sourcesContent[i]) {
          ...
          try {
            if (options.debug) console.log(PLUGIN_NAME + '-write: No source content for "' + sourceMap.sources[i] + '". Loading from file.');
            sourceMap.sourcesContent[i] = stripBom(fs.readFileSync(sourcePath, 'utf8'));
          } catch (e) {
            if (options.debug) console.warn(PLUGIN_NAME + '-write: source file not found: ' + sourcePath);
          }
        }
      }
    ...
    if (options.addComment) file.contents = Buffer.concat([file.contents, new Buffer(comment)]);
