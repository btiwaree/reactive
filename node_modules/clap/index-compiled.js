...
function stringLength(str) {
  return str.replace(ansiRegex, '').replace(reAstral, ' ').length;
}
...
function assign(dest, source) {
  for (var key in source) if (hasOwnProperty.call(source, key)) dest[key] = source[key];
...
function parseParams(str) {
  ...
  if (left) throw new SyntaxError('Bad parameter description: ' + str);
...
var Option = function (usage, description) {
  ...
  if (!this.long) throw new SyntaxError('Usage has no long name: ' + usage);
  ...
  if (left) throw new SyntaxError('Bad usage description for option: ' + usage);

  if (!this.name) this.name = this.long;
...
function createOption(usage, description, opt_1, opt_2) {
  ...
  if (arguments.length == 3) {
    if (opt_1 && opt_1.constructor === Object) {
      for (var key in opt_1) if (key == 'normalize' || key == 'defValue' || key == 'beforeInit') option[key] = opt_1[key];

      // old name for `beforeInit` setting is `hot`
      if (opt_1.hot) option.beforeInit = true;
    } else {
      if (typeof opt_1 == 'function') option.normalize = opt_1;else option.defValue = opt_1;
    }
  }

  if (arguments.length == 4) {
    if (typeof opt_1 == 'function') option.normalize = opt_1;
...
function addOptionToCommand(command, option) {
  ...
  if (option.short) {
    ...
    if (commandOption) throw new SyntaxError('Short option name -' + option.short + ' already in use by ' + commandOption.usage + ' ' + commandOption.description);
  ...
  if (commandOption) throw new SyntaxError('Long option --' + option.long + ' already in use by ' + commandOption.usage + ' ' + commandOption.description);
  ...
  if (commandOption) throw new SyntaxError('Name option ' + option.camelName + ' already in use by ' + commandOption.usage + ' ' + commandOption.description);

  command.options[option.camelName] = option;

  // set default value
  if (typeof option.defValue != 'undefined') command.setOption(option.camelName, option.defValue, true);
  ...
  function processOption(option, command) {
    ...
    if (option.maxArgsCount) {
      for (var j = 0; j < option.maxArgsCount; j++) {
        ...
        if (!nextToken || nextToken[0] == '-') break;

        params.push(args[++i]);
      }

      if (params.length < option.minArgsCount) throw new SyntaxError('Option ' + token + ' should be used with at least ' + option.minArgsCount + ' argument(s)\nUsage: ' + option.usage);
    } else {
    ...
    if (token[0] == '-') {
      ...
      if (token[1] == '-') {
        ...
        if (!option) {
          ...
          if (suggestPoint) return findVariants(command, token);else throw new SyntaxError('Unknown option: ' + token);
        }
        ...
        if (!/^-[a-zA-Z]+$/.test(token)) throw new SyntaxError('Wrong short option sequence: ' + token);

        if (token.length == 2) {
          option = command.short[token[1]];

          if (!option) throw new SyntaxError('Unknown short option name: -' + token[1]);
          ...
          for (var j = 1; j < token.length; j++) {
            ...
            if (!option) throw new SyntaxError('Unknown short option name: -' + token[j]);

            if (option.maxArgsCount) throw new SyntaxError('Non-boolean option -' + token[j] + ' can\'t be used in short option sequence: ' + token);
      ...
      if (command.commands[token] && (!command.params || commandArgs.length >= command.params.minArgsCount)) {
        ...
        if (command.params && resultToken.args.length < command.params.minArgsCount) throw new SyntaxError('Missed required argument(s) for command `' + command.name + '`');
        ...
        if (suggestPoint) return findVariants(command, token);else throw new SyntaxError('Unknown command: ' + token);
      }
    }
  }

  if (suggest) {
    if (collectArgs || noSuggestions) return [];

    return findVariants(command, suggestStartsWith);
  } else {
    if (!noOptionsYet) resultToken.literalArgs = commandArgs;else resultToken.args = commandArgs;
...
function setFunctionFactory(name) {
  return function (fn) {
    ...
    if (this[property] !== noop) throw new SyntaxError('Method `' + name + '` could be invoked only once');

    if (typeof fn != 'function') throw new SyntaxError('Value for `' + name + '` method should be a function');
...
var Command = function (name, params) {
  ...
  try {
    if (params) this.params = parseParams(params);
  } catch (e) {
...
Command.prototype = {
  ...
  shortcut: function (usage, description, fn, opt_1, opt_2) {
    if (typeof fn != 'function') throw new SyntaxError('fn should be a function');
    ...
    option.normalize = function (value) {
      ...
      for (var name in values) if (hasOwnProperty.call(values, name)) if (hasOwnProperty.call(command.options, name)) command.setOption(name, values[name]);else command.values[name] = values[name];
  ...
  setOption: function (name, value, isDefault) {
    if (!this.hasOption(name)) throw new SyntaxError('Option `' + name + '` is not defined');

    var option = this.options[name];
    var newValue = Array.isArray(value) ? option.normalize.apply(this, value) : option.normalize.call(this, value);

    this.values[name] = option.maxArgsCount ? newValue : value;

    if (isDefault && !hasOwnProperty.call(this.defaults_, name)) this.defaults_[name] = this.values[name];
  },
  setOptions: function (values) {
    for (var name in values) if (hasOwnProperty.call(values, name) && this.hasOption(name)) this.setOption(name, values[name]);
  },
  reset: function () {
    this.values = {};
  ...
  command: function (nameOrCommand, params) {
    ...
    if (nameOrCommand instanceof Command) {
      ...
      if (!/^[a-zA-Z][a-zA-Z0-9\-\_]*$/.test(name)) throw new SyntaxError('Wrong command name: ' + name);
    }
  ...
  version: function (version, usage, description) {
    if (this.version_) throw new SyntaxError('Version for command could be set only once');

    this.version_ = version;
    this.option(usage || '-v, --version', description || 'Output version', function () {
  ...
  description: function (description) {
    if (this.description_) throw new SyntaxError('Description for command could be set only once');
  ...
  parse: function (args, suggest) {
    ...
    if (!errorHandler) return processArgs(this, args, suggest);else try {
  ...
  run: function (args, context) {
    ...
    for (var i = 0; i < commands.length; i++) {
      ...
      if (prevCommand) prevCommand.delegate_(command);

      // apply beforeInit options
      command.setOptions(item.options.reduce(function (res, entry) {
        if (entry.option.beforeInit) res[entry.option.camelName] = entry.value;
        return res;
      }, {}));

      command.init_(item.args);

      if (item.args.length) command.args_(item.args);
    ...
    if (command) return command.action_(item.args, item.literalArgs);
  },
  ...
  normalize: function (values) {
    ...
    for (var name in this.values) if (hasOwnProperty.call(this.values, name)) result[name] = hasOwnProperty.call(values, name) && hasOwnProperty(this.options, name) ? this.options[name].normalize.call(this, values[name]) : this.values[name];

    for (var name in values) if (hasOwnProperty.call(values, name) && !hasOwnProperty.call(this.options, name)) result[name] = values[name];
...
function showCommandHelp(command) {
  ...
  function commandsHelp() {
    ...
    var lines = Object.keys(command.commands).sort().map(function (name) {
      ...
      var line = {
        name: chalk.green(name) + chalk.gray(subcommand.params ? ' ' + args(subcommand) : ''),
  ...
  function optionsHelp() {
    ...
    var lines = Object.keys(command.long).sort().map(function (name) {
      ...
      var line = {
        name: option.usage.replace(/^(?:-., |)/, function (m) {
    ...
    return ['', 'Options:', '', lines.map(function (line) {
      return '  ' + pad(maxNameLength, line.name) + '  ' + breakByLines(line.description, maxNameLength + 4);
    }).join('\n'), ''].join('\n');
  }
  ...
  if (command.description_) output.push(command.description_ + '\n');

  output.push('Usage:\n\n  ' + chalk.cyan(commandsPath ? commandsPath.join(' ') : command.name) + (command.params ? ' ' + chalk.magenta(args(command)) : '') + (command.hasOptions() ? ' [' + chalk.yellow('options') + ']' : '') + (command.hasCommands() ? ' [' + chalk.green('command') + ']' : ''), commandsHelp() + optionsHelp());
...
module.exports = {
  ...
  error: function (fn) {
    if (errorHandler) throw new SyntaxError('Error handler should be set only once');

    if (typeof fn != 'function') throw new SyntaxError('Error handler should be a function');
