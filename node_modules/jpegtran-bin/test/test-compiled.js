...
it('rebuild the jpegtran binaries', function (cb) {
	var cfg = ['./configure --disable-shared', '--prefix="' + tmp + '" --bindir="' + tmp + '"'].join(' ');

	new BinBuild().src('https://downloads.sourceforge.net/project/libjpeg-turbo/1.4.2/libjpeg-turbo-1.4.2.tar.gz').cmd('touch configure.ac aclocal.m4 configure Makefile.am Makefile.in').cmd(cfg).cmd('make install').run(function (err) {
		if (err) {
			cb(err);
			return;
		}
...
it('return path to binary and verify that it is working', function () {
	var args = ['-outfile', path.join(tmp, 'test.jpg'), path.join(__dirname, 'fixtures/test.jpg')];
