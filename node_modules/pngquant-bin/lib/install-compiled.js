...
bin.run(['--version'], function (err) {
	if (err) {
		...
		var builder = new BinBuild().src('https://github.com/pornel/pngquant/archive/2.5.2.tar.gz').cmd('./configure --prefix="' + bin.dest() + '"').cmd('make install BINPREFIX="' + bin.dest() + '"');

		return builder.run(function (err) {
			if (err) {
				err.message = ['pngquant failed to build, make sure that', libpng + ' is installed'].join(' ');
